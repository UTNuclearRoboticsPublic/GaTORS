#include "gators/game_manager.hpp"
...
{
    sensor_msgs::msg::PointCloud2 move_map;
    board_utils::loadCloudasMsg(move_dir, move_map);
    sensor_msgs::msg::PointCloud2 repair_map;
...
void GameManager::instantiateBoard (const std::string &move_dir, const float &move_discretization, sensor_msgs::msg::PointCloud2 &move_board, const std::string &repair_dir, const float &repair_discretization, sensor_msgs::msg::PointCloud2 &repair_board)
{
    sensor_msgs::msg::PointCloud2 move_map;
    board_utils::loadCloudasMsg(move_dir, move_map);
    sensor_msgs::msg::PointCloud2 repair_map;
...
MoveOptions GameManager::listMovesfromNodeConstrained (agents::Robot &player, RepairBoard &board)
{
    ...
    if ((player.remaining_movement > 0) && (player.remaining_coverage == player.get_max_turn_coverage())) {
        ...
        for (std::size_t i = 0; i < movement_edges.size(); i++) {
            Action action;
    ...
    if (player.remaining_coverage > 0) {
        ...
        for (std::size_t i = 0; i < repair_edges.size(); i++) {
            Action action;
